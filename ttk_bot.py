...
welcome_text = f'Здраствуйте, я бот-помощник ТТК, Чтобы войти напишите "Войти как клиент ТТК". Если хотите заключить новый догвор, пожалуйста напишите "Заключить новый договор"' 
user_contract_enter_text = f'Пожалуйста введите свой номер договора, чтобы я понял кто вы' 
user_contact_info_text = f'Укажите свой контактный номер и адрес для подключения услуги' 
bot_not_understand_text = f'Извини, я тебя не понял. Давай еще раз'
user_wrong = f'Неправильно, попробуйте еще раз'
...
def enter_as_client(message):
    print(message.text)
    if message.text.isdigit() and len(message.text) == 9 and message.text[:6] == '516':
        print('succses_enter')
